...
/**
 * Shared pierce query handler.
 *
 * @author Kimi Liu
 * @since Java 17+
 */
public final class PierceQueryHandler {

    /**
     * Creates a pierce query handler.
     */
    private PierceQueryHandler() {
    ...
    public static String querySelectorExpression(String rootExpression, String selector) {
        return QueryHandler.lanciaExpression(
                "Lancia.pierceQuerySelector(" + QueryHandler.root(rootExpression) + ","
                        + QueryHandler.literal(QueryHandler.assertSelector(selector, "pierce query")) + ")");
    }
    ...
    public static String querySelectorAllExpression(String rootExpression, String selector) {
        return QueryHandler.lanciaExpression(
                "Array.from(Lancia.pierceQuerySelectorAll(" + QueryHandler.root(rootExpression) + ","
                        + QueryHandler.literal(QueryHandler.assertSelector(selector, "pierce query")) + "))");
    }
