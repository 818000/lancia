...
/**
 * Shared XPath query handler.
 *
 * @author Kimi Liu
 * @since Java 17+
 */
public final class XPathQueryHandler {

    /**
     * Creates an XPath query handler.
     */
    private XPathQueryHandler() {
    ...
    public static String querySelectorExpression(String rootExpression, String selector) {
        return querySelectorAllExpression(rootExpression, selector, 1);
    }
    ...
    public static String querySelectorAllExpression(String rootExpression, String selector) {
        return querySelectorAllExpression(rootExpression, selector, 0);
    }
     ...
     * @return expression
     */
    private static String querySelectorAllExpression(String rootExpression, String selector, int maxResults) {
        String body = "const out=[];for(const node of Lancia.xpathQuerySelectorAll(" + QueryHandler.root(rootExpression)
                + "," + QueryHandler.literal(QueryHandler.assertSelector(selector, "XPath")) + "," + maxResults
                + ")){out.push(node);}return " + (maxResults == 1 ? "out.length?out[0]:null" : "out") + ";";
        return QueryHandler.lanciaBlock(body);
    }
