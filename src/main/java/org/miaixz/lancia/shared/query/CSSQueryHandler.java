...
/**
 * Shared CSS query handler.
 *
 * @author Kimi Liu
 * @since Java 17+
 */
public final class CSSQueryHandler {

    /**
     * Creates a CSS query handler.
     */
    private CSSQueryHandler() {
    ...
    public static String querySelectorExpression(String rootExpression, String selector) {
        return QueryHandler.lanciaExpression(
                "Lancia.cssQuerySelector(" + QueryHandler.root(rootExpression) + ","
                        + QueryHandler.literal(QueryHandler.assertSelector(selector, "CSS")) + ")");
    }
    ...
    public static String querySelectorAllExpression(String rootExpression, String selector) {
        return QueryHandler.lanciaExpression(
                "Array.from(Lancia.cssQuerySelectorAll(" + QueryHandler.root(rootExpression) + ","
                        + QueryHandler.literal(QueryHandler.assertSelector(selector, "CSS")) + "))");
    }
